...
#ifdef WIN32GUI
		else if (!STRCMP("/cos", com_token))
			compile_on_start = flag;
		else if (!STRCMP("/errorguide", com_token))
		{
			src = COM_Parse(src);
			if (!src)
				return;
			strcpy(errorguide, com_token);
		}
		else if (!STRCMP("/editor", com_token))
		{
			src = COM_Parse(src);
			if (!src)
					return;
				strcpy(editor, com_token);
		}
		else
			ExtractFilename(com_token, progssrc);
