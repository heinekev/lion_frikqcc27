...
//	*(int *)-4 = 0;	// dump to debugger
	...
	conditional = false;
#ifdef WIN32GUI
	ErrorPrint(strings + s_file, pr_source_line, num, "Error Q%i %s(%i): %s", num, strings + s_file, pr_source_line, string);
#else
...
void PR_ParseWarning (int level, char *warn, ...)
{
	...
	pr_warning_count++;
#ifdef WIN32GUI
	ErrorPrint(strings + s_file, pr_source_line, level, "Warning Q%i %s(%i): %s", level, strings + s_file, pr_source_line, string);
#else
